    ...
    protected function getDataQCM($forUser = true)
    {
        try {
            if($forUser) {
                ...
                $data = $this->prepare("SELECT q.*, c.id AS data_id, c.reponse_choisi, c.nb_reponse, c.id_user FROM ".TABLE_QCM." q JOIN ".TABLE_COLLECT_DATA." c ON c.id_qcm = q.id WHERE c.id_user = ? ORDER BY data_id ASC");
